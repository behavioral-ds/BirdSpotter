...
class BirdSpotter:
    ...
    def setWord2VecEmbeddings(self, embeddings=None, forceReload=True):
        ...
        if embeddings is None:
            ...
            if os.path.isfile('./glove.twitter.27B.200d.txt'):
                print("Loading GloVe twitter.27B.200d Word2Vec Embeddings...")
                ...
                try:
                    ...
                    filename = wget.download('http://nlp.stanford.edu/data/glove.twitter.27B.zip')
                    print('\n')
                    with zipfile.ZipFile(filename, 'r') as zip_ref:
                        zip_ref.extractall('./')
                    print("Loading downloaded GloVe twitter.27B.200d Word2Vec Embeddings...")
                    with open('./glove.twitter.27B.200d.txt',"r") as f:
                        model = {}
                        for line in tqdm(f):
                            splitLine = line.split()
